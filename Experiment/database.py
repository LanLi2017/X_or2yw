...
def dep_tb(v):
    """
    capture dependency relationship: column input, column output

    :param v: block of hybrid prov
    ...
    for i, v in enumerate(hybrid_js):
        ...
        try:
            ...
            sql_insert_singleedit = f''' INSERT INTO {tb_name}(history_id, step_index,op)
                              VALUES({history_id},{step_th},"singleedit"); '''
...
def main():
    ...
    # create database
    DB = r"DataCleaning_wf_2.db"
    sql_f = open('workflow_2.sql', 'w')
    # sql_f.write(f'''sqlite3 {DB};'''+'\n')
    ...
    sql_create_trans_params_table = """CREATE TABLE IF NOT EXISTS transformation_params(
                          id INTEGER PRIMARY KEY AUTOINCREMENT,
                          history_id integer,
                          param_name text,
                          param_value text,
    ...
    sql_f.write(sql_create_trans_params_table + '\n')
    ...
    for i, value in enumerate(hybrid_prov):
        ...
        if 'operation' in value:
            op = value['operation']
            opname = op['op'].replace('/', '').replace('-', '')
            sql_insert_transformation = f'''INSERT INTO transformation(history_id, step_index,op,column_input,column_output) VALUES ({history_id}, {step_id}, "{opname}","{column_input}","{column_output}");'''
            sql_f.write(sql_insert_transformation+'\n')
            for k, v in op.items():
                sql_insert_trans_param = f'''INSERT INTO transformation_params(history_id,param_name, param_value)VALUES({history_id}, "{k}", "{v}");'''
                sql_f.write(sql_insert_trans_param + '\n')
        else:
            sql_insert_transformation = f'''INSERT INTO transformation(history_id, step_index,op,column_input,column_output) VALUES ({history_id}, {step_id}, "singleedit","{column_input}","{column_output}");'''
            sql_f.write(sql_insert_transformation + '\n')
            sql_insert_trans_param = f'''INSERT INTO transformation_params(history_id,param_name, param_value)VALUES({history_id}, "", "");'''
            sql_f.write(sql_insert_trans_param + '\n')

    # insert_transformation_tb(hybrid_prov, sql_f)
    insert_change_tb(hybrid_prov, sql_f)
    # insert_dep_tb(hybrid_prov, sql_f)
